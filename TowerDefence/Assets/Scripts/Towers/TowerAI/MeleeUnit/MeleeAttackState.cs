...
public class MeleeAttackState : MeleeBaseState
{
    ...
    private float range = 10f; // Raycast range
    private float aoeRadius = 5f; // AoE radius for damage
    private float damageAmount = 35f; // Amount of damage for the AoE attack

    

    public MeleeAttackState(GameObject go)
    {
        shootLocation = GameObject.FindWithTag("MeleeShootLocation").transform.position;
    }
